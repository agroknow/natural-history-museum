...
//            div.push('</DIV>');
        ...
        var msg = 'Search the #{repName}'.interpolate({repName: "Natural History Museum of Crete"});
...
function parseQueryString(initUpdate){
    ...
    if(!plainText.blank()){
        ...
        clauses.push({language:'anyOf',expression:'provider:edunhmc'}); //changed from organicedunet
...
new Ajax.JSONRequest(SERVICE_URL, {
 ...
 Jaml.register('rbcriteria', function(data)
               ...
               if(data.val=="edunhmc"){label = "Natural History Museum";} //in future it will created through mapping for providers in order to have more readable labels
