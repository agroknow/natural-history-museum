	...
	if (!FINDER_INITIALIZED) {
		...
		for (var i=0;i<FACET_TOKENS.length;i++){
			...
			div.push('</DIV></DIV>');
                ...
                div.push('<DIV id="rb_Clear" class="rbClear"><input type="submit" name="id" value="Reset filters" onclick="initialSearch();pagination_hide();">');
                div.push('</DIV>');
...
function parseQueryString(initUpdate){
  ...
  if(plainText.blank())
  {
	  clauses.push({language:'anyOf',expression:'collection:n.h.m.c. - univ. of crete'});
  }
    clauses.push({language:'anyOf',expression:'provider:n.h.m.c. - univ. of crete'});
  return clauses;
}
				  ...
				  if(item.keywords == undefined || item.keywords == '')
					  ...
					  try {item.keywords = item.keywords.split("&#044; ");} catch(e) {}
                                   ...
                                   */
                                      ...
                                      //alert(item.format + 'item.location=' + item.location);
...
//element.insert(Jaml.render('rbcriteria',it2));
                                                    ...
                                                    if(fld=="provider")
                                                    {
                                                        if (it2.val=="n.h.m.c. - univ. of crete"){element.insert(Jaml.render('rbcriteria',it2));}
                                                    }
                                                    else if(fld=="format")
                                                    {
                                                        if (it2.val=="IMAGE"){element.insert(Jaml.render('rbcriteria',it2));}
                                                    }
                                                    else
                                                    {
                                                        element.insert(Jaml.render('rbcriteria',it2));
...
function initializeJamlTemplates(){
    ...
    Jaml.register('thumb_pres', function(data) {
     a({href: data.location,title: data.title , target: '_blank'}, img({src:data.location, height:"90", width:"auto" }))
...
Jaml.render('first_title',function(data){
 ...
 Jaml.register('rbcriteria', function(data)
 {
               ...
               var label = data.val;
               //if(data.val=="organicedunet")
               if(data.val=="n.h.m.c. - univ. of crete"){label = "Natural History Museum of Crete";} //in future it will created through mapping for providers in order to have more readable labels
